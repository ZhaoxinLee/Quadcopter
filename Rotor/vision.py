...
class Vision(object):
        ...
        if self.isFireWire():
            if self.isUpdating():
                ...
                if self.isCOMDrawingEnabled():
                    self.COMDrawing(self.agent[0].x,self.agent[0].y,self.agent[1].x,self.agent[1].y,self.agent[2].x,self.agent[2].y,self.agent[3].x,self.agent[3].y)
    ...
    def processObjectDetection(self,imageFiltered,imageOriginal):
        ...
        imageProcessed = algorithm(imageFiltered,imageOriginal,self.agent,1) # pass instances of Agent class if you want to update its info
    ...
    def COMDrawing(self,x1,y1,x2,y2,x3,y3,x4,y4):
        ...
        if mouseX != 0 and mouseY != 0:
            ...
            self.addDrawing('arrow',(int(x),int(y),self.desiredX,self.desiredY,1))
