...
class Vision2(object):
    def __init__(self,field,index,type,guid=0000000000000000,buffersize=10):
        ...
        # drawings
        self._isFieldDrawingEnabled = False   # draw field indicators
        ...
        if self.isFireWire():
            ...
            self.cap = cv2.VideoCapture('rotor_Mar12.mp4')
        ...
        if self.isFireWire():
            if self.isUpdating():
                ...
                if self.isAgentDrawingEnabled():
                    self.agentDrawing(self.agent[0].x,self.agent[0].y)
    ...
    def processFilters(self,image):
        for item in (self.filterRouting[0],self.filterRouting[2]):
    ...
    def processObjectDetection(self,imageFiltered,imageOriginal):
        ...
        imageProcessed = algorithm(imageFiltered,imageOriginal,self.agent,2) # pass instances of Agent class if you want to update its info
