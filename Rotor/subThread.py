...
class SubThread(QThread):
    ...
    def __init__(self,field,vision,vision2,joystick=None,parent=None,):
    ...
    def rotor(self):
        ...
        while True:
            ...
            realZ = self.vision2.agent[0].y
            if realZ >350:
                uniformZ = 0.05*(realZ-350)
            # desiredZ = 270
            # frequency = round(1+0.01*(realZ-desiredZ),2)
            # print(realZ,frequency)
            # self.field.setFrequency(frequency)
            # theta = 2 * pi * frequency * t
            ...
            # set up gradient field to achieve pulling
            realCOM = [self.vision.COMx,self.vision.COMy]
            desiredCOM = self.vision.getDesiredCOM()
            vector = [desiredCOM[0]-realCOM[0],desiredCOM[1]-realCOM[1]]
            ...
            # d = self.params[3]
            #
            # if self.preCOMx != None:
            #     gradientX = p*vector[0]+d*(realCOM[0]-self.preCOMx)/(t-self.pretime)
            #     if gradientX > 10:
            #         gradientX = 10
            #     elif gradientX < -10:
            #         gradientX = -10
            #
            #     gradientY = -p*vector[1]-d*(realCOM[1]-self.preCOMy)/(t-self.pretime)
            #     if gradientY > 10:
            #         gradientY = 10
            #     elif gradientY < -10:
            #         gradientY = -10
            #     print(round(gradientX,2),round(gradientY,2))
            # else:
            #     gradientX = 0
            #     gradientY = 0

            gradientX = p*vector[0]
            if gradientX > 12:
                gradientX = 12
            elif gradientX < -12:
                gradientX = -12

            gradientY = -p*vector[1]
            if gradientY > 12:
                gradientY = 12
            elif gradientY < -12:
                gradientY = -12
            ...
            self.storeLastCOM(self.vision.COMx,self.vision.COMy,t)

            if self.counter == 1000:
                self.vision.setTime(round(t,2))
                self.counter = 0
            self.counter += 1
    ...
    def tilt(self):
        ...
        while True:
            ...
            # realZ = self.vision2.agent[0].y
            # desiredZ = 270
            # frequency = 8+0.01*(realZ-desiredZ)
            # print(realZ,frequency)
            # self.field.setFrequency(frequency)
            # theta = 2 * pi * frequency * t
            ...
            self.field.setFrequency(self.params[0])
            self.field.setMagnitude(self.params[1])
            #tilt = radians(self.params[2])
            #phi = radians(self.params[3])
            theta = 2 * pi * self.params[0] * t


            realCOM = [self.vision.COMx,self.vision.COMy]
            desiredCOM = self.vision.getDesiredCOM()
            vector = [realCOM[0]-desiredCOM[0],realCOM[1]-desiredCOM[1]]
            length = sqrt(vector[0]**2+vector[1]**2)

            if self.params[0] > 0:
                if length > 30:
                    tilt = radians(30+0.2*(length-30))
                    if tilt > radians(50):
                        tilt = radians(50)
                else:
                    tilt = radians(1*length)
                    if tilt < radians(10):
                        tilt = 0

                phi = atan2(-vector[1],vector[0])-pi/2
                if tilt == 0:
                    phi = 0
                #print(phi,round(degrees(phi),2))
            else:
                if length > 30:
                    tilt = -radians(30+0.2*(length-30))
                    if tilt < -radians(50):
            ...
            fieldX = self.params[1] * (cos(theta)*cos(phi)-cos(tilt)*sin(theta)*sin(phi))
            fieldY = self.params[1]*(cos(theta)*sin(phi)+cos(tilt)*sin(theta)*cos(phi))
            fieldZ = self.params[1]*sin(tilt) * sin(theta)
            ...
            self.field.setXGradient(fieldX,gradientX)
            self.field.setYGradient(fieldY,gradientY)
            self.field.setZGradient(fieldZ,gradientZ)
            self.field.setMagnitude(self.params[0])
